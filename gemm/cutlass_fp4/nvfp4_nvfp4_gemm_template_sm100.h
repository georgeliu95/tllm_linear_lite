 ...
 *
 * This file is derived from TensorRT-LLM's kernels/cutlass_kernels/fp4_gemm/nvfp4_nvfp4_gemm_template_sm100.h.
 *
 * Changes from the original:
 *   - Replaced includes: tensorrt_llm/common/config.h -> trtllm_cutlass_compat.h
 *   - Replaced includes: tensorrt_llm/common/envUtils.h -> removed (getEnvEnablePDL in compat header)
 *   - Replaced includes: tensorrt_llm/kernels/archCondition.h -> archCondition.h (local copy)
 *   - Replaced includes: tensorrt_llm/kernels/cutlass_kernels/cutlass_type_conversion.h
 *     -> cutlass_type_conversion.h (minimal local version, no NvInferRuntime.h)
 *   - Kept:    genericFp4GemmKernelLauncher template (SM100/SM103), INSTANTIATE_FP4_GEMM_KERNEL_LAUNCHER macro
 */
