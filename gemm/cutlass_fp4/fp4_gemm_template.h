...
/*
 * [Standalone NVFP4-only modification]
 *
 * This file is derived from TensorRT-LLM's kernels/cutlass_kernels/fp4_gemm/fp4_gemm_template.h.
 *
 * Changes from the original:
 *   - Replaced includes: tensorrt_llm/common/{config,cudaUtils,logger}.h
 *     -> trtllm_cutlass_compat.h (provides namespace macros, getSMVersion, check_cuda_error,
 *        TLLM_LOG_DEBUG/WARNING as no-ops)
 *   - Replaced includes: cutlass_extensions/gemm_configs.h -> gemm_configs.h (local copy)
 *   - Replaced includes: ../include/fp4_gemm.h -> fp4_gemm.h (local copy)
 *   - Removed: #include "mxfp8_mxfp4_gemm_template_sm100.h" (MXFP8 not needed for NVFP4)
 *   - Removed: dispatchMXFP8xMXFP4GemmClusterShapeSm100() (MXFP8 dispatch)
 *   - Removed: dispatchMXFP8xMXFP4GemmCTAShapeSm100() (MXFP8 dispatch)
 *   - Removed: MXFP8 branch in CutlassFp4GemmRunner::dispatchToArch()
 *   - Removed: MXFP8 smem skip logic in CutlassFp4GemmRunner::getConfigs()
 *   - Kept:    All NVFP4xNVFP4 dispatch functions (Sm100, Sm103, Sm120)
 *   - Kept:    CutlassFp4GemmRunner constructor, destructor, gemm, getConfigs, getWorkspaceSize
 */
...
// MXFP8 dispatch functions removed for NVFP4-only build
...
size_t CutlassFp4GemmRunner<T, fp4GemmType>::dispatchToArch(T* D, void const* A, void const* B, void const* input_sf,
    ...
    int* occupancy)
{
    // MXFP8 path removed for NVFP4-only build
